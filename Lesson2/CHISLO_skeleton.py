#более простого выхода избежать ошибки я не обнаружила. для чего применять "длина строки" я тоже не понимаю.
chislo = input ("Введите целое число от 0 до 99: ")
if int(chislo)<0 or int(chislo)>99:
	print ("Пожалуйста, ведите число в диапазоне от 0 до 99.")
elif int(chislo) == 0:
	print ("Ноль")
elif int(chislo) == 1:
	print ("Один")
elif int(chislo) == 2:
	print ("Два")
elif int(chislo) == 3:
	print ("Три")
elif int(chislo) == 4:
	print ("Четыре")
elif int(chislo) == 5:
	print ("Пять")
elif int(chislo) == 6:
	print ("Шесть")
elif int(chislo) == 7:
	print ("Семь")
elif int(chislo) == 8:
	print ("Восемь")
elif int(chislo) == 9:
	print ("Девять")
elif int(chislo) == 10:
	print ("Десять")
elif int(chislo) == 11:
	print ("Одиннадцать")
elif int(chislo) == 12:
	print ("Двенадцать")
elif int(chislo) == 13:
	print ("Тринадцать")
elif int(chislo) == 14:
	print ("Четырнадцать")
elif int(chislo) == 15:
	print ("Пятнадцать")
elif int(chislo) == 16:
	print ("Шестнадцать")
elif int(chislo) == 17:
	print ("Семнадцать")
elif int(chislo) == 18:
	print ("Восемнадцать")
elif int(chislo) == 19:
	print ("Девятнадцать")
elif int(chislo) == 20:
	print ("Двадцать")
elif int(chislo) == 21:
	print ("Двадцать один")
elif int(chislo) == 22:
	print ("Двадцать два")
elif int(chislo) == 23:
	print ("Двадцать три")
elif int(chislo) == 24:
	print ("Двадцать четыре")
elif int(chislo) == 25:
	print ("Двадцать пять")
elif int(chislo) == 26:
	print ("Двадцать шесть")
elif int(chislo) == 27:
	print ("Двадцать семь")
elif int(chislo) == 28:
	print ("Двадцать восемь")
elif int(chislo) == 29:
	print ("Двадцать девять")
elif int(chislo) == 30:
	print ("Тридцать")
elif int(chislo) == 31:
	print ("Тридцать один")
elif int(chislo) == 32:
	print ("Тридцать два")
elif int(chislo) == 33:
	print ("Тридцать три")
elif int(chislo) == 34:
	print ("Тридцать четыре")
elif int(chislo) == 35:
	print ("Тридцать пять")
elif int(chislo) == 36:
	print ("Тридцать шесть")
elif int(chislo) == 37:
	print ("Тридцать семь")
elif int(chislo) == 38:
	print ("Тридцать восемь")
elif int(chislo) == 39:
	print ("Тридцать девять")
elif int(chislo) == 40:
	print ("Сорок")
elif int(chislo) == 41:
	print ("Сорок один")
elif int(chislo) == 42:
	print ("Сорок два")
elif int(chislo) == 43:
	print ("Сорок три")
elif int(chislo) == 44:
	print ("Сорок четыре")
elif int(chislo) == 45:
	print ("Сорок пять")
elif int(chislo) == 46:
	print ("Сорок шесть")
elif int(chislo) == 47:
	print ("Сорок семь")
elif int(chislo) == 48:
	print ("Сорок восемь")
elif int(chislo) == 49:
	print ("Сорок девять")
elif int(chislo) == 50:
	print ("Пятьдесят")
elif int(chislo) == 51:
	print ("Пятьдесят один") 
elif int(chislo) == 52:
	print ("Пятьдесят два")
elif int(chislo) == 53:
	print ("Пятьдесят три")
elif int(chislo) == 54:
	print ("Пятьдесят четыре")
elif int(chislo) == 55:
	print ("Пятьдесят пять")
elif int(chislo) == 56:
	print ("Пятьдесят шесть")
elif int(chislo) == 57:
	print ("Пятьдесят семь")
elif int(chislo) == 58:
	print ("Пятьдесят восемь")
elif int(chislo) == 59:
	print ("Пятьдесят девять")
elif int(chislo) == 60:
	print ("Шестьдесят")
elif int(chislo) == 61:
	print ("Шестьдесят один")
elif int(chislo) == 62:
	print ("Шестьдесят два")
elif int(chislo) == 63:
	print ("Шестьдесят три")
elif int(chislo) == 64:
	print ("Шестьдесят четыре")
elif int(chislo) == 65:
	print ("Шестьдесят пять")
elif int(chislo) == 66:
	print ("Шестьдесят шесть")
elif int(chislo) == 67:
	print ("Шестьдесят семь")
elif int(chislo) == 68:
	print ("Шестьдесят восемь")
elif int(chislo) == 69:
	print ("Шестьдесят девять")
elif int(chislo) == 70:
	print ("Семьдесят")
elif int(chislo) == 71:
	print ("Семьдесят один")
elif int(chislo) == 72:
	print ("Семьдесят два")
elif int(chislo) == 73:
	print ("Семьдесят три")
elif int(chislo) == 74:
	print ("Семьдесят четыре")
elif int(chislo) == 75:
	print ("Семьдесят пять")
elif int(chislo) == 76:
	print ("Семьдесят шесть")
elif int(chislo) == 77:
	print ("Семьдесят семь")
elif int(chislo) == 78:
	print ("Семьдесят восемь")
elif int(chislo) == 79:
	print ("Семьдесят девять")
elif int(chislo) == 80:
	print ("Восемьдесят")
elif int(chislo) == 81:
	print ("Восемьдесят один")
elif int(chislo) == 82:
	print ("Восемьдесят два")
elif int(chislo) == 83:
	print ("Восемьдесят три")
elif int(chislo) == 84:
	print ("Восемьдесят четыре")
elif int(chislo) == 85:
	print ("Восемьдесят пять")
elif int(chislo) == 86:
	print ("Восемьдесят шесть")
elif int(chislo) == 87:
	print ("Восемьдесят семь")
elif int(chislo) == 88:
	print ("Восемьдесят восемь")
elif int(chislo) == 89:
	print ("Восемьдесят девять")
elif int(chislo) == 90:
	print ("Девяносто")
elif int(chislo) == 91:
	print ("Девяносто один")
elif int(chislo) == 92:
	print ("Девяносто два")
elif int(chislo) == 93:
	print ("Девяносто три")
elif int(chislo) == 94:
	print ("Девяносто четыре")
elif int(chislo) == 95:
	print ("Девяносто пять")
elif int(chislo) == 96:
	print ("Девяносто шесть")
elif int(chislo) == 97:
	print ("Девяносто семь")
elif int(chislo) == 98:
	print ("Девяносто восемь")
elif int(chislo) == 99:
	print ("Девяносто девять")
